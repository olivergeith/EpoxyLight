...
void EpoxyLight::drawWheel(int step) {
...
void EpoxyLight::drawWheelColorful(int millies) {
	animationStep = animationStep + 1;
	if (animationStep >= 256)
		animationStep = 0;
	for (int i = 0; i < strip.numPixels(); i++) {
		strip.setPixelColor(i, Wheel(animationStep + i *128 / strip.numPixels()));
	}
	strip.show();
}
...
void EpoxyLight::drawLauflichtRandomColor(int millies) {
	if (millies % 50 == 0) {
...
uint32_t EpoxyLight::getColorForLetter(int index) {
	int lettercolor = 144;
	switch (index) {
	default:
	case 0: // D
...
uint32_t EpoxyLight::getColorForStripIndex(int index) {
	int lettercolor = 156;
	switch (index) {
	default:
	case 0:
...
void EpoxyLight::drawRandomSegment(int millies, int modulo) {
...
void EpoxyLight::drawRandomSegmentUSA(int millies, int modulo) {
...
void EpoxyLight::drawDoubleLauflichtWheeled(int millies) {
	if (millies % 100 == 0) {
		animationStep = animationStep + 4;
		if (animationStep >= 256)
			animationStep = 0;
		...
		for (int i = 0; i < strip.numPixels(); i++) {
			if (i == lauflichtStep //
					|| i == strip.numPixels() /4 + lauflichtStep //
					|| i == strip.numPixels() / 2 + lauflichtStep //
					|| i == strip.numPixels() /4*3 + lauflichtStep //
			) {
				strip.setPixelColor(i, laufLichtColor);
			} else {
				strip.setPixelColor(i, 0, 0, 0);
